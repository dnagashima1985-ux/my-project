...
ので、キーワードとして強い側を後半に置く。`hook` は表紙の煽り（`typo` では斜めの
帯に入る）、`copy` は下部の帯に流れる1行になる。`diagram_*` は枠組みが段階モデルの
ときだけ使う（`board` レイアウトの図になる）。無ければ週の升目にフォールバック
するので省略してよい。
...
- **build_cover.py** — `cover/<slug>-<layout>.jpg` を1600×2560で3案。
  **3案は色違いではなく、構図から別のデザイン**（`pitch` 芝と白線／`board`
  戦術ボード／`typo` 図版なしの極太タイポ）。どれも下部に帯を敷く——ここが
  自費出版に見えるかどうかの分かれ目。和文フォントは実際に使う文字だけを
  Google Fontsから取ってくるので、どんなタイトルでも化けない。ブラウザが無い
  環境ではPillowで同じ図版を描く（`COVER_RENDERER=pillow` で明示指定）
